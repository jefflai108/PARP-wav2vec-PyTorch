...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if hasattr(cfg.task, 'mask_file'):
            feature_extractor_zero_rate, bert_zero_rate, total_zero_rate = see_weight_rate(model.w2v_encoder.w2v_model)
            print('in the trainer: wav2vec 2.0 feature extractor zero rate %f' % feature_extractor_zero_rate)
            print('in the trainer: wav2vec 2.0 BERT zero rate %f' % bert_zero_rate)
            print('in the trainer: wav2vec 2.0 total_zero_rate %f' % total_zero_rate)

            # apply masking
            # hack: move both model and mask to cuda and apply this operation
            #if cfg.task.mask_file is not None:
            #    model.to('cpu')
            #    self.mask = torch.load(cfg.task.mask_file, map_location='cpu')
            #    apply_pruning_mask(model.w2v_encoder.w2v_model, self.mask)
            #    feature_extractor_zero_rate, bert_zero_rate, total_zero_rate = see_weight_rate(model.w2v_encoder.w2v_model)
            ...
            #    print(model.w2v_encoder.w2v_model.post_extract_proj.weight.device)
            #    print(model.w2v_encoder.w2v_model.post_extract_proj.bias.device)
            #    model.to(self.device)
            #    print(model.w2v_encoder.w2v_model.post_extract_proj.weight.device)
            #    print(model.w2v_encoder.w2v_model.post_extract_proj.bias.device)
            #    s = list(model.w2v_encoder.w2v_model.named_parameters())
            #    t = list(model.w2v_encoder.w2v_model.named_buffers())
            #    print([(x[0], x[1].device) for x in s])
            #    print('=========')
            #    print([(x[0], x[1].device) for x in t])
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if self.is_data_parallel_master:  # only save one checkpoint
            ...
            if hasattr(self.cfg.task, 'mask_file'):
                feature_extractor_zero_rate, bert_zero_rate, total_zero_rate = see_weight_rate(self.get_model().w2v_encoder.w2v_model)
        ...
        if bexists:
            ...
            try:
                ...
                if hasattr(self.cfg.task, 'mask_file') and self.cfg.task.mask_file is not None:
                    ...
                    #mask = torch.load(self.cfg.task.mask_file, map_location=self.device)
                    #apply_pruning_mask(self.get_model().w2v_encoder.w2v_model, mask)
                    feature_extractor_zero_rate, bert_zero_rate, total_zero_rate = see_weight_rate(self.get_model().w2v_encoder.w2v_model)
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        self._set_seed()
        self.model.train()
        #print(self._model.w2v_encoder.w2v_model.post_extract_proj.weight.device)
        #print(self._model.w2v_encoder.w2v_model.post_extract_proj.bias.device)
        #print(list(self._model.w2v_encoder.w2v_model.named_buffers()))

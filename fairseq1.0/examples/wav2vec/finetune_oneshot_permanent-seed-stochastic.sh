...
if [ $stage -eq 3 ]; then
    ...
    expdir=exp/${pretrained_model}-finetune-${train_subset}-oneshot-permanent-seed${seed}/${mask_name}
...
if [ $stage -eq 5 ]; then
    ...
    valid_subset=dev-other
    expdir=exp/${pretrained_model}-finetune-${train_subset}-oneshot-permanent-seed${seed}-stochastic-scale0.003/${mask_name}
    datadir=data/${train_subset}
    mask_file=exp/${pretrained_model}-finetune-${train_subset}/oneshot-cpc/${mask_name}.pt

...
if [ $stage -eq 0 ]; then
    for split in dev-clean dev-other train-clean-100 test-clean test-other train-clean-360 train-960 train-1h train-10h train-10min-0 train-10min-1 train-10min-2 train-10min-3 train-10min-4 train-10min-5; do
        mkdir -p data/${split}
        python -u wav2vec_manifest.py /nobackup/users/clai24/data/LibriSpeech/${split} --dest data/${split} --ext flac --valid-percent 0
    done
fi
...
if [ $stage -eq 2 ]; then
    # fine-tuning hubert_base_ls960 on 100 hr and validate on dev-other
    # other hyper-param: https://github.com/pytorch/fairseq/blob/master/examples/wav2vec/config/finetuning/base_100h.yaml
    pretrained_model=hubert_base_ls960
...
if [ $stage -eq 3 ]; then
    # fine-tuning hubert_base_ls960 on 10 hr and validate on dev-other
    pretrained_model=hubert_base_ls960
...
if [ $stage -eq 4 ]; then
    # fine-tuning hubert_base_ls960 on 1 hr and validate on dev-other
    pretrained_model=hubert_base_ls960
...
if [ $stage -eq 5 ]; then
    # fine-tuning hubert_base_ls960 on 10 min and validate on dev-other
    pretrained_model=hubert_base_ls960

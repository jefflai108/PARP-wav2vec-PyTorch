...
if [ $stage -eq 0 ]; then
    for lan in es fr it ky nl ru sv_SE tr tt zh_TW all; do
        # filter orig train tsv file
        #cp -r data/commonvoice-${lan}-train data/commonvoice-${lan}-train-10min
        python filter-cv-10min.py data/commonvoice-${lan}-train/${lan}-train.tsv data/commonvoice-${lan}-train/${lan}-train.ltr data/commonvoice-${lan}-train-10min/${lan}-train.tsv data/commonvoice-${lan}-train-10min/${lan}-train.ltr
        cp data/commonvoice-${lan}-train-10min/${lan}-train.tsv data/commonvoice-${lan}-train-10min/train.tsv
        cp data/commonvoice-${lan}-train-10min/${lan}-train.ltr data/commonvoice-${lan}-train-10min/${lan}-train.wrd
    done
    exit 0
fi
...
datadir=data/commonvoice-${language}-train-10min
...
if [ $stage -eq 4 ]; then
    ...
    expdir=exp/${pretrained_model}-finetune-commonvoice-10min-${language}-2

...
if [ $stage -eq 2 ]; then
    ...
    for source in test-other test-clean; do
    for target in train-clean-100 train-clean-360 train-960 train-1h train-10h train-10min-0 train-10min-1 train-10min-2 train-10min-3 train-10min-4 train-10min-5; do
        head -10 data/$source/${source}.ltr > data/$target/${source}-10.ltr
        head -10 data/$source/${source}.wrd > data/$target/${source}-10.wrd
        head -11 data/$source/train.tsv > data/$target/${source}-10.tsv
    done; done
fi
